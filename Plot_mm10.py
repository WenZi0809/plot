...
#--------------------------------------------------------------------------
# Hi-C Related ...
HiCFolder = '/public/home/3dgenome/wenzi/mm10/Hi-c'
...
TADSource = os.path.join(TADFolder, 'mESC-HindIII-hierdomain.txt')
HiCData = np.load(HiCSource)
TADData = analyze.TAD(TADSource).data
chrom = ['1']
interval = 50
# Output Figure Name
oPrefix = 'mESC-Heatmaps-1k_Signals-mm10_new'
#--------------------------------------------------------------------------
# One-dimensional Tracks
sigFolder = '/public/home/3dgenome/wenzi/mm10/npy-1k'
Sigs =  ['CTCF','new-CTCF','Med1','Med12','Smc1','Smc3','Rad21-rep2','CapG','CapD3']
...
#        Matrix = np.ma.array(Matrix, mask = Bool)
        ...
        ## Domain Boundaries
        mask = (tads['end'] > startHiC ) & (tads['start'] < EndHiC )
        extract = tads[mask]
        pairs = [(bi['start'] - startHiC, bi['end'] - startHiC)
                 for bi in extract]
        for corner in pairs:
            if (corner[0] <= 0):
